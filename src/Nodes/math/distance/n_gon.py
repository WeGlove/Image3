...
class NGon(Node):
    ...
    def __init__(self):
        ...
        self.position_noso = NodeSocket("Position")
        self.shift_noso = NodeSocket("Shift")
        super().__init__([self.n, self.position_noso, self.shift_noso])
    ...
    def produce(self):
        ...
        for i in range(int(n)):
            point = self.position_noso.get().produce() + torch.matmul(unit, rotation_2D(2 * math.pi * i / n, self.defaults.device))[0]
            points.append(point)

        map = torch.zeros((self.defaults.width, self.defaults.height))
        for i in range(int(n)):
            if i == n - 1:
                a, b = two_points_to_linear(points[i], points[0], self.position_noso.get().produce())
            else:
                a, b = two_points_to_linear(points[i], points[i + 1], self.position_noso.get().produce())
