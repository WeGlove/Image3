...
class MassAlpha(Node):
    ...
    def __init__(self, node_id, factory_id, frame_counter, device):
        ...
        self.images = []
        super().__init__(node_id, factory_id, "Mass Alpha", frame_counter, [self.noso_images, self.noso_compositor], device, [])

    def produce(self):
        mask = self.noso_compositor.get().produce() % 1 * len(self.images)
